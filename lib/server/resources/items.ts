...
import type { ItemDetailDto, ItemQueryInput, ItemRowDto, PropValue } from "@/lib/shared";
import { prisma } from "../prisma";
import * as itemScope from "./item-scope";
...
/**
 * Resource reads. Every query starts from item-scope.ts's visibleItemWhere — no
 ...
 * server-side filtering/sort/pagination. Every row here is one this user may see; scope
 * is applied BEFORE this query runs, not filtered out of a fuller result.
 */
export async function search(userId: string, query: ItemQueryInput): Promise<ItemRowDto[]> {
  const scope = await itemScope.visibleItemWhere(userId);
...
export async function getOne(userId: string, id: string): Promise<ItemDetailDto> {
  await itemScope.assertCanSeeItem(userId, id);
  const row = await prisma.item.findUniqueOrThrow({ where: { id }, include: ROW_INCLUDE });
  return {
    ...toRowDto(row),
