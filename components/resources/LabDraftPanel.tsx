...
type StageableKind = "setName" | "setStatus" | "setQuantity" | "deleteItem";

const STAGEABLE_KINDS: StageableKind[] = ["setName", "setStatus", "setQuantity", "deleteItem"];

function describeChange(row: ItemDraftChangeDto, itemName: (id: string) => string): string {
  if (row.targetKind === "IDEAL") {
    const p = row.payload as { categoryId: string; qty: number };
    return `Ideal target: ${p.qty} of this category`;
  }
  const p = row.payload as ItemChangeInput;
  const label = CHANGE_LABEL[p.kind] ?? p.kind;
  if (p.kind === "createItem") return label;
 ...
 * common corrections (rename, status, quantity, delete) rather than every
 * ItemChangeKind the server already accepts — createItem/moveInTree/images/custom
...
export function LabDraftPanel({ labItemId, labName, onClose }: { labItemId: string; labName: string; onClose: () => void }) {
  ...
  async function stage() {
    ...
    try {
      ...
      if (kind === "setName") change = { kind: "setName", itemIds: [targetId], value };
    ...
    <Modal title={`Draft — ${labName}`} onClose={onClose} width="720px">
      ...
      <Panel title="Stage a change">
        <div className="p-12 flex flex-wrap items-end gap-8">
          ...
          <label className="flex flex-col gap-3">
            <span className="text-9.5 uppercase tracking-label text-faint">Item</span>
          ...
          {kind !== "deleteItem" && (
            <label className="flex flex-col gap-3">
              <span className="text-9.5 uppercase tracking-label text-faint">Value</span>
              {kind === "setStatus" ? (
          ...
          <Button variant="primary" onClick={stage} disabled={busy || !targetId}>
            Stage
          </Button>
        </div>
      </Panel>
          ...
          <div className="flex flex-col">
            {openVisible.map((d) => (
              <div key={d.id} className="flex items-center justify-between px-14 py-8 border-b border-border last:border-0 text-11">
                <span>{describeChange(d, itemName)}</span>
