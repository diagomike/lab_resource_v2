 ...
 *
 * `next/navigation`'s `useSearchParams()` is read-only, unlike react-router's mutable
 * version this was originally built against — see `useTableUrlState` below for how the
 * functional-updater contract (`setParams(prev => next)`) is reconstructed on top of
 * `router.replace()`.
 */
...
export function useTableUrlState(opts: CodecOptions): [TableState, TableStateActions] {
  ...
  const state = useMemo(() => decodeTableState(searchParams, codec), [searchParams, codec]);

  // `useSearchParams()` is read-only, unlike react-router's `setSearchParams(fn, {replace})`,
  // which this was originally built against and which hands `fn` the CURRENT params no
  // matter how many times it's called in one tick. Reconstructing that here needs a ref: it
  // tracks the params any pending `router.replace()` call is about to produce, so two
  // `setParams` calls made synchronously back-to-back (as several of the actions below can
  // do via `write`) each see the other's effect rather than both computing off the same
  // stale render-time `searchParams` and one silently clobbering the other.
  ...
  const setParams = useCallback(
    (updater: (prev: URLSearchParams) => URLSearchParams) => {
      const next = updater(new URLSearchParams(paramsRef.current.toString()));
